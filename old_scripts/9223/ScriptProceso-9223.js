...
if (CO_PAGBOT  == 1) { 
    if (r_no_nombre == null || r_no_nombre.trim() == ''){
        MSG.PUSH_TO_USER(USUARI.co_usuari, MSG_TYPE_WARNING,'ALERTA',"Ingrese nombre de Personal Autorizado");
        return OK('NONE', null, null, null);
    } else if (r_co_docide == null || r_co_docide.trim() == '') {
        MSG.PUSH_TO_USER(USUARI.co_usuari, MSG_TYPE_WARNING,'ALERTA',"Ingrese DNI de Personal Autorizado");
        return OK('NONE', null, null, null);
    } else if (r_co_tipdoc == 'D' & r_co_docide.length !== 8) {
        MSG.PUSH_TO_USER(USUARI.co_usuari, MSG_TYPE_WARNING,'ALERTA',"DNI debe tener 8 caracteres.");
        return OK('NONE', null, null, null);
    } else if (r_co_tipdoc == 'D' & isNaN(r_co_docide) == true) {
        MSG.PUSH_TO_USER(USUARI.co_usuari, MSG_TYPE_WARNING,'ALERTA',"DNI debe contener solo números.");
        return OK('NONE', null, null, null);
    } else if (r_co_tipdoc == 'E' & r_co_docide.length !== 11) {
        MSG.PUSH_TO_USER(USUARI.co_usuari, MSG_TYPE_WARNING,'ALERTA',"Carnet de extranjeria debe tener 11 caracteres.");
          return OK('NONE', null, null, null);
    } else if (r_co_tipdoc == 'E' & isNaN(r_co_docide) == true) {
        MSG.PUSH_TO_USER(USUARI.co_usuari, MSG_TYPE_WARNING,'ALERTA',"Carnet de extranjeria debe contener solo números.");
        return OK('NONE', null, null, null);
    } else if (r_no_carper == null || r_no_carper.trim() == '') {
        MSG.PUSH_TO_USER(USUARI.co_usuari, MSG_TYPE_WARNING,'ALERTA',"Ingrese cargo de Personal Autorizado");
        return OK('NONE', null, null, null);
    } else if (r_nu_telefo == null || r_nu_telefo.trim() == '') {
        MSG.PUSH_TO_USER(USUARI.co_usuari, MSG_TYPE_WARNING,'ALERTA',"Ingrese teléfono de Personal Autorizado");
        return OK('NONE', null, null, null);
    } else if (isNaN(r_nu_telefo) == true) {
        MSG.PUSH_TO_USER(USUARI.co_usuari, MSG_TYPE_WARNING,'ALERTA',"El número de teléfono debe contener solo números ");
        return OK('NONE', null, null, null);
    } else if (r_nu_telefo.length !== 9) {
        MSG.PUSH_TO_USER(USUARI.co_usuari, MSG_TYPE_WARNING,'ALERTA',"El número de teléfono debe contener 9 digitos.");
        return OK('NONE', null, null, null);
    } else if (r_no_correo == null || r_no_correo.trim() == '') {
        MSG.PUSH_TO_USER(USUARI.co_usuari, MSG_TYPE_WARNING,'ALERTA',"Ingrese correo de Personal Autorizado");
        return OK('NONE', null, null, null);
    } else if ( !expr.test(r_no_correo) ){
        MSG.PUSH_TO_USER(USUARI.co_usuari, MSG_TYPE_WARNING,'ALERTA',"La dirección de correo " + r_no_correo + " es incorrecta.");
        return OK('NONE', null, null, null);
    } else {
        // QUERYS:
        var v_tx_query = 
        ...
        if(v_va_resqry.co_estado == '00'){
            ...
            MSG.PUSH_TO_USER(USUARI.co_usuari, MSG_TYPE_SUCCESS, 'CORRECTO', "El registro se realizó correctamente.", CO_CONTEN, true);
            ...
            MSG.PUSH_TO_USER(USUARI.co_usuari, MSG_TYPE_ERROR,'ALERTA',"Ha ocurrido un problema al realizar el registro." + v_tx_query.status, CO_CONTEN, true);
    ...
    if (v_va_resqry.cantidad == 0){
      MSG.PUSH_TO_USER(USUARI.co_usuari, MSG_TYPE_WARNING,'ALERTA',"Ingrese personal autorizado.", CO_CONTEN, true);
      return OK('NONE', null, null, null);
    } else {
